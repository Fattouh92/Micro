...
public class TomasuloAlg {
	...
	public TomasuloAlg(int ibSize, int loadRsSize, int storeRsSize,
			int addSubRsSize, int nandRsSize, int multRsSize, int addiRsSize,
			int jmpJalrRetRsSize, int beqRsSize, int robSize, int loadCyc,
			int storeCyc, int addSubCyc, int nandCyc, int multCyc, int addiCyc,
			int jmpJalrRetCyc, int beqCyc) {
	...
	public void start(ArrayList<Register> regs, ICache iCache, Cache cache,
			Memory memory, Register pc) {
		// public void start(ArrayList<Register> regs, String[] mem, Register
		// pc){
			...
			if (!rob.isEmpty()) {
				if (rob.peek()[1].equals("Y")) {
					if (rob.peek()[3].equals("O")) {
						fu.CommitOP(Integer.parseInt(rob.peek()[0]),
								getRegOrg(regs, rob.peek()[2]));
					} else if (rob.peek()[3].equals("S")) {
						String[] split = rob.peek()[2].split(" ");
						String address = split[0];
						String offset = split[1];
						fu.CommitStore(memory, Integer.parseInt(rob.peek()[0]),
								getRegOrg(regs, address),
				...
				if (ib.get(i, 1).equals("")) {
					...
					if (!rob.isFull()) {
						...
						case "load":
							if (loadRs.size() < loadRsSize) {
								...
								rob.enqueue(robRec);
								ib.modify(i, 2,
										Integer.toString(rob.nItems - 1));
								int regIndex = getRegIndex(regs, regsVal[0]);
								regTable[regIndex] = rob.nItems - 1;
								boolean valid = this.freeDest(regs, regTable,
										regsVal[1]);
								String[] entry;
								if (valid) {
									entry = new String[] {
											"Y",
											Integer.toString(getRegOrg(regs,
													regsVal[1]).getValue()),
											"",
											Integer.toString(rob.nItems - 1),
											Integer.toString(loadCyc) };
								} else {
									entry = new String[] {
											"Y",
											"",
											Integer.toString(regTable[getRegIndex(
													regs, regsVal[1])]),
											Integer.toString(rob.nItems - 1),
											Integer.toString(loadCyc) };
								}
						...
						case "sub":
							if (addSubRs.size() < addSubRsSize) {
								...
								boolean valid = this.freeDest(regs, regTable,
										regsVal[1]);
								boolean valid2 = this.freeDest(regs, regTable,
										regsVal[2]);
								String[] entry;
								if (valid && valid2) {
									entry = new String[] {
							...
							if (jmpJalrRetRs.size() < jmpJalrRetRsSize) {
								...
								if (valid) {
									entry = new String[] {
											...
											Integer.toString(getRegOrg(regs,
													regsVal[0]).getValue()),
											Integer.toString(pc.getValue()),
											"",
											Integer.toString(rob.nItems - 1),
											Integer.toString(jmpJalrRetCyc) };
								} else {
									entry = new String[] {
											"Y",
											"",
					...
					case "load":
						...
						if (loadRs.get(index)[4].equals(Integer.toString(0))) {
							...
							if (!rob.get(
									Integer.parseInt(loadRs.get(index)[2]), 0)
									.equals("")) {
								String[] temp = loadRs.get(index);
								temp[1] = Integer
										.toString(getRegOrg(
												regs,
												rob.get(Integer.parseInt(loadRs
														.get(index)[2]), 2))
												.getValue());
								temp[2] = "";
								loadRs.set(index, temp);
					...
					case "store":
						...
						if (storeRs.get(index)[4].equals(Integer.toString(0))) {
							...
							if (!rob.get(
									Integer.parseInt(storeRs.get(index)[2]), 0)
									.equals("")) {
								String[] temp = storeRs.get(index);
								temp[1] = Integer
										.toString(getRegOrg(
					...
					case "sub":
						...
						if (addSubRs.get(index)[6].equals(Integer.toString(0))) {
							...
							// addSubRs.get(index)[6]);
						} else if (addSubRs.get(index)[3].equals("")
								&& addSubRs.get(index)[4].equals("")) {
							...
							if (addSubRs.get(index)[1].equals("")) {
								if (!rob.get(
										Integer.parseInt(addSubRs.get(index)[3]),
										0).equals("")) {
									String[] temp = addSubRs.get(index);
									temp[1] = Integer.toString(getRegOrg(
											regs,
											rob.get(Integer.parseInt(addSubRs
													.get(index)[3]), 2))
											.getValue());
									temp[3] = "";
									addSubRs.set(index, temp);
							...
							if (addSubRs.get(index)[2].equals("")) {
								if (!rob.get(
										Integer.parseInt(addSubRs.get(index)[4]),
										0).equals("")) {
									String[] temp = addSubRs.get(index);
									temp[2] = Integer.toString(getRegOrg(
											regs,
											rob.get(Integer.parseInt(addSubRs
													.get(index)[4]), 2))
											.getValue());
					...
					case "nand":
						...
						if (nandRs.get(index)[6].equals(Integer.toString(0))) {
							ib.modify(i, 1, "executed");
						} else if (nandRs.get(index)[3].equals("")
								&& nandRs.get(index)[4].equals("")) {
							...
							if (nandRs.get(index)[1].equals("")) {
								if (!rob.get(
										Integer.parseInt(nandRs.get(index)[3]),
										0).equals("")) {
									String[] temp = nandRs.get(index);
									temp[1] = Integer.toString(getRegOrg(
											regs,
											rob.get(Integer.parseInt(nandRs
													.get(index)[3]), 2))
											.getValue());
									temp[3] = "";
									nandRs.set(index, temp);
							...
							if (nandRs.get(index)[2].equals("")) {
								if (!rob.get(
										Integer.parseInt(nandRs.get(index)[4]),
										0).equals("")) {
									String[] temp = nandRs.get(index);
									temp[2] = Integer.toString(getRegOrg(
											regs,
					...
					case "mult":
						...
						if (multRs.get(index)[6].equals(Integer.toString(0))) {
							ib.modify(i, 1, "executed");
						} else if (multRs.get(index)[3].equals("")
								&& multRs.get(index)[4].equals("")) {
							...
							if (multRs.get(index)[1].equals("")) {
								if (!rob.get(
										Integer.parseInt(multRs.get(index)[3]),
										0).equals("")) {
									String[] temp = multRs.get(index);
									temp[1] = Integer.toString(getRegOrg(
											regs,
											rob.get(Integer.parseInt(multRs
													.get(index)[3]), 2))
											.getValue());
									temp[3] = "";
									multRs.set(index, temp);
							...
							if (multRs.get(index)[2].equals("")) {
								if (!rob.get(
										Integer.parseInt(multRs.get(index)[4]),
										0).equals("")) {
									String[] temp = multRs.get(index);
									temp[2] = Integer.toString(getRegOrg(
											regs,
					...
					case "addi":
						...
						if (addiRs.get(index)[4].equals(Integer.toString(0))) {
							...
							if (!rob.get(
									Integer.parseInt(addiRs.get(index)[2]), 0)
									.equals("")) {
								String[] temp = addiRs.get(index);
								temp[1] = Integer
										.toString(getRegOrg(
												regs,
												rob.get(Integer.parseInt(addiRs
														.get(index)[2]), 2))
												.getValue());
								temp[2] = "";
								addiRs.set(index, temp);
					...
					case "jmp":
						...
						if (jmpJalrRetRs.get(index)[5].equals(Integer
								.toString(0))) {
							ib.modify(i, 1, "executed");
							...
							if (!rob.get(
									Integer.parseInt(jmpJalrRetRs.get(index)[3]),
									0).equals("")) {
								String[] temp = jmpJalrRetRs.get(index);
								temp[1] = Integer
										.toString(getRegOrg(
					...
					case "beq":
						...
						if (beqRs.get(index)[7].equals(Integer.toString(0))) {
							ib.modify(i, 1, "executed");
						} else if (beqRs.get(index)[4].equals("")
								&& beqRs.get(index)[5].equals("")) {
							...
							if (beqRs.get(index)[1].equals("")) {
								if (!rob.get(
										Integer.parseInt(beqRs.get(index)[4]),
										0).equals("")) {
									String[] temp = beqRs.get(index);
									temp[1] = Integer.toString(getRegOrg(
											regs,
											rob.get(Integer.parseInt(beqRs
													.get(index)[4]), 2))
											.getValue());
									temp[4] = "";
									beqRs.set(index, temp);
							...
							if (beqRs.get(index)[2].equals("")) {
								if (!rob.get(
										Integer.parseInt(beqRs.get(index)[5]),
										0).equals("")) {
									String[] temp = beqRs.get(index);
									temp[2] = Integer.toString(getRegOrg(
											regs,
					...
					if (canWrite || Integer.parseInt(ib.get(i, 2)) <= bindex) {
						...
						case "load":
							...
							result = fu.Load(memory,
									getRegOrg(regs, regsVal[0]),
									Integer.parseInt(loadRs.get(index)[1]),
									Integer.parseInt(regsVal[2]));
							rob.modify(Integer.parseInt(loadRs.get(index)[3]),
									0, Integer.toString(result));
							rob.modify(Integer.parseInt(loadRs.get(index)[3]),
									1, "Y");
							if (regTable[getRegIndex(regs,
									rob.get(Integer.parseInt(ib.get(i, 2)), 2))] == Integer
									.parseInt(ib.get(i, 2))) {
								regTable[getRegIndex(regs, rob.get(
										Integer.parseInt(ib.get(i, 2)), 2))] = -1;
							}
							loadRs.remove(index);
						...
						case "store":
							...
							result = fu.Store(memory,
									Integer.parseInt(storeRs.get(index)[1]),
									getRegOrg(regs, regsVal[1]),
									Integer.parseInt(regsVal[2]));
							rob.modify(Integer.parseInt(storeRs.get(index)[3]),
									0, Integer.toString(result));
							rob.modify(Integer.parseInt(storeRs.get(index)[3]),
									1, "Y");
						...
						case "sub":
							...
							result = fu.AddSub(op, getRegOrg(regs, regsVal[0]),
									Integer.parseInt(addSubRs.get(index)[1]),
									Integer.parseInt(addSubRs.get(index)[2]));
							rob.modify(
									Integer.parseInt(addSubRs.get(index)[5]),
									0, Integer.toString(result));
							rob.modify(
									Integer.parseInt(addSubRs.get(index)[5]),
									1, "Y");
						...
						case "nand":
							...
							result = fu.NAND(getRegOrg(regs, regsVal[0]),
									Integer.parseInt(nandRs.get(index)[1]),
									Integer.parseInt(nandRs.get(index)[2]));
							rob.modify(Integer.parseInt(nandRs.get(index)[5]),
									0, Integer.toString(result));
							rob.modify(Integer.parseInt(nandRs.get(index)[5]),
									1, "Y");
							if (regTable[getRegIndex(regs,
						...
						case "mult":
							...
							result = fu.Multiply(getRegOrg(regs, regsVal[0]),
									Integer.parseInt(multRs.get(index)[1]),
									Integer.parseInt(multRs.get(index)[2]));
							rob.modify(Integer.parseInt(multRs.get(index)[5]),
									0, Integer.toString(result));
							rob.modify(Integer.parseInt(multRs.get(index)[5]),
									1, "Y");
							if (regTable[getRegIndex(regs,
						...
						case "addi":
							...
							result = fu.Addi(getRegOrg(regs, regsVal[0]),
									Integer.parseInt(addiRs.get(index)[1]),
									Integer.parseInt(regsVal[2]));
							rob.modify(Integer.parseInt(addiRs.get(index)[3]),
									0, Integer.toString(result));
							rob.modify(Integer.parseInt(addiRs.get(index)[3]),
									1, "Y");
							if (regTable[getRegIndex(regs,
						...
						case "jmp":
							...
							result = fu.jump(pc, Integer.parseInt(jmpJalrRetRs
									.get(index)[2]), Integer
									.parseInt(jmpJalrRetRs.get(index)[1]),
									Integer.parseInt(regsVal[1]));
							rob.modify(
									Integer.parseInt(jmpJalrRetRs.get(index)[4]),
									0, Integer.toString(result));
							rob.modify(
									Integer.parseInt(jmpJalrRetRs.get(index)[4]),
									1, "Y");
						...
						case "beq":
							...
							int pc3 = pc.getValue();
							result = fu.BEQ(pc,
									Integer.parseInt(beqRs.get(index)[3]),
									Integer.parseInt(beqRs.get(index)[1]),
									Integer.parseInt(beqRs.get(index)[2]),
									Integer.parseInt(regsVal[2]));
							if (result != pc3) {
								...
								for (int a = 0; a < jmpJalrRetRs.size(); a++) {
									if (Integer
											.parseInt(jmpJalrRetRs.get(a)[4]) > bindex) {
							...
							rob.modify(Integer.parseInt(beqRs.get(index)[6]),
									0, Integer.toString(result));
							rob.modify(Integer.parseInt(beqRs.get(index)[6]),
									1, "Y");
							if (regTable[getRegIndex(regs,
						...
						case "jalr":
							...
							result = fu
									.Jalr(pc, Integer.parseInt(jmpJalrRetRs
											.get(index)[2]),
											getRegOrg(regs, regsVal[0]),
											Integer.parseInt(jmpJalrRetRs
													.get(index)[1]));
							rob.modify(
									Integer.parseInt(jmpJalrRetRs.get(index)[4]),
									0, Integer.toString(result));
						...
						case "ret":
							...
							result = fu.Return(Integer.parseInt(jmpJalrRetRs
									.get(index)[1]), pc);
							rob.modify(
									Integer.parseInt(jmpJalrRetRs.get(index)[4]),
									0, Integer.toString(result));
			...
			if (!ib.isFull() && addIns) {
				...
				try {
					if (timeForFetch == 0) {
						...
						timeForFetch = iCache.read(
								Integer.toBinaryString(pc.getValue()), 1);
					}
					// ins = mem[pc.getValue()];
	...
	public boolean freeDest(ArrayList<Register> regs, int[] regTable,
			String destReg) {
	...
	public static void main(String[] args) {
		TomasuloAlg t = new TomasuloAlg(8, 5, 2, 2, 2, 2, 1, 2, 8, 8, 2, 12, 6,
				2, 2, 2, 2, 2);
		...
		String[] mem = { "add F16 R2 R3", "sub F2 F4 F8", "nand F10 F0 F6",
				"sub F6 F8 F2" };
		memo.writeData("0", mem[0]);
		memo.writeData("2", mem[1]);
		memo.writeData("4", mem[2]);
		memo.writeData("6", mem[3]);
		t.start(regs, new ICache(128, 16, 1, 64, 4, 2, 0, 0, 0, 3, 0, 0, 1, 1,
				1, 0, 0, 0, 0, 2, memo), new Cache(128, 16, 1, 64, 4, 2, 0, 0,
				0, 3, 0, 0, 10, 1, 1, 0, 0, 0, 0, 2, memo), memo, pc);
	}
}
